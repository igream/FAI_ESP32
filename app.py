...
try:
    ...
    collection = db['Datos']
    # Verificar conexión intentando listar colecciones
    client.server_info()  # Esto lanza una excepción si no hay conexión
...
def simulador():
    ...
    while True:
        try:
            ...
            result = collection.insert_one(dato, write_concern={'w': 'majority'})
            logger.info(f"[{ahora}] Dato insertado con ID: {result.inserted_id}, Dato: {dato}")
        except Exception as e:
